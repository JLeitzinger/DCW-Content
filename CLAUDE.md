...
### **NPC (Monster)** Actors
Real `dccworldNPC` stat blocks (health, power, abilities, cr) from the Dungeon-Crawler-World system, illustrated with the DCSS `assets/tiles/monster/` pack - "mobs" in the sense of easily-defeated-but-dangerous-in-number, and the only enemy type that's still a flat formula rather than a real character (see **Character (Elite/Boss/Friendly NPC) Actors** below for everything else). Authored in `data/monsters-manifest.json` (`npm run generate:monsters` → `src/packs/monsters/`, packed by `npm run pack:monsters`). `generate-monsters.mjs` only turns **`band: "minion"`** rows into standalone Monster actors now - `elite`/`boss` rows are still resolved from the same manifest, but purely as archetype *input* to `CharacterGenerator.mjs`, not as their own Monster entry. Two consumers share the minion rows: the Monsters compendium itself (browsable/importable like any other Actor), and `scripts/lib/level-gen/MonsterGenerator.mjs`, which auto-populates generated floors' rooms with Tokens referencing it - "generating a floor" also generates its encounters now, no separate step.
...
- `combatSkill` - one of the real combat/magic skills in `data/skills-manifest.json` (Slash/Pierce/Blunt/Shoot/Throw/Cast/Channel) - this monster's one attack. Picked for flavor fit, not mechanically tied to `primaryAbility` (e.g. a dex-primary fire vortex can still "Shoot" a flame jet).
- `attackName` - flavor name for the embedded weapon item this generates (e.g. "Rotting Claws", "Death Bolt") - see below.
- `biography` - one line of flavor text, shown on the Actor's Description tab.

**CR/stat formula** (computed, never hand-set - see `computeStats`/`computeAbilities`/`computeCombat` in monster-roster.mjs): `cr = round(tier * bandMultiplier)` where minion=1, elite=1.8, boss=3. `health.max = 8 + cr*6`, `power.max = 4 + cr*3`. Abilities start at 10 flat; `primaryAbility` becomes `10+cr`, `secondaryAbility` (if set) becomes `10+ceil(cr/2)`. This mirrors `dccworldNPC`'s own `xp = cr*cr*100` already being cr-driven - CR is this roster's one difficulty knob, same as everywhere else in the system.

**Combat capability - every monster gets two embedded Items (`generate-monsters.mjs`'s `buildCombatItems`), or it's just a stat block with nothing to roll:** an "attack" weapon (`attackName`, equipped, grants `combatSkill` at level = cr - a monster has no race/class/skill-item stack to draw from the way a PC does) and a "Natural Defenses" armor (grants Block if `con` is its primary/secondary ability, else Dodge, at level = ceil(cr/2), plus flat `damageReduction`). Damage dice scale by tier (d6/d8/d10); ranged `combatSkill`s (Shoot/Cast/Channel) get `range: "60 feet"`, everything else `"melee"`.

**Critical pack-format gotcha if `pack-monsters.mjs` is ever touched:** Foundry's LevelDB backend does NOT store an Actor's embedded `items` inline in its own record - confirmed by reading Foundry's own server source
(`app/dist/database/fields-extensions.mjs`'s `EmbeddedCollectionField#_dbWrite`/`expandEmbedded`). The actor record must store `items` as an array of item **id strings only**; each item's real data goes in a separate `actors.items` sublevel keyed `<actorId>.<itemId>`. Writing items inline (the obvious naive approach - and what earlier versions of this script did) passes `npm run pack` and even opens fine as a live document immediately after packing, but Foundry's one-time compendium migration on the *next* world launch treats every item as unresolvable and silently rewrites the pack with `items: []` - permanently deleting every monster's weapon/armor the first time any world loads it. `pack-monsters.mjs` uses `classic-level`'s own `db.sublevel('actors.items', ...)` to write the correct format (verified byte-compatible with Foundry's own key prefixing since both pin identical `classic-level`/`abstract-level` versions - check `package.json`/`node_modules` if either is ever bumped). If any other document type in this repo ever gets embedded collections (e.g. Scene tokens are fine since Foundry's own `SceneBuilder.mjs` output already goes through the normal generate→pack path without embedded-in-embedded collections), re-verify this same way before assuming a naive inline write is safe.

**Known Foundry timing quirk, not a content bug:** a monster's `xp`, ability `.mod`/`.label`, and `aggregatedSkills` can show as zero/empty immediately after importing or placing a token for the first time *within an already-running browser session* - Foundry's derived-data preparation silently fails once on first construction in that specific timing window, but a single browser page reload permanently fixes it for that actor (the underlying data was always correct; only the in-memory derived computation was stale). Not something this content generator can control - see the `npc-xp-display-bug` memory if this needs deeper investigation someday.
...
### **Character (Elite/Boss/Friendly NPC)** Actors
Real `dccworldCharacter` actors - a race + class + equipped weapon/armor + (if the class casts) spells + feats, the same content pool a player draws from, not a synthetic formula. Covers everything that isn't a mob: `elite`/`boss`-band encounters and a brand-new **friendly NPC** category. Built entirely at floor-generation time by `scripts/lib/level-gen/CharacterGenerator.mjs` - there's no `data/characters-manifest.json` to hand-author; a Characters compendium entry only exists because some floor's generation run produced it, so regenerating floors regenerates its cast too.

**Why a race/class build instead of a CR formula:** `dccworldNPC` (the Monster mob type above) is a flat stat block with no race/class/skill-item stack. `dccworldCharacter` already has everything needed - `prepareDerivedData()` (`Dungeon-Crawler-World/module/data/actor-character.mjs`) computes HP/Stamina/Mana from owned class/race items and applies racial ability bonuses automatically, exactly like a real PC - so a GM-controlled character actor "just works" the moment it owns the right items, no ownership-by-a-player-user required.

**The three cases, all built via shared helpers in `CharacterGenerator.mjs`:**
- **`buildBoss(rng, id, theme, tier, archetype)`** - the floor's one boss. `archetype` is the boss-band row `monster-roster.mjs` resolves from `data/monsters-manifest.json` (same curated art/theme/primaryAbility/combatSkill as before - only the mechanical build changed). Built once per floor in `generate-floors.mjs`, **before** the story graph, so its name can be woven into the climax milestone text (see `StoryGenerator.mjs`). Gets a personal name (`data/narrative-lexicon.json`'s `names`) + a `{threat}`-templated title (`bossTitles`), and +1 extra feature beyond its race/class's own.
- **`buildEliteCharacter(rng, id, idPrefix, theme, tier, archetype)`** - one elite encounter, built fresh per placement (`MonsterGenerator.mjs`'s room loop) so two elites on the same floor are never identical builds. Keeps the archetype's own `name`/`img`/`biography` as its role label - no personal name.
- **`buildFriendlyNpc(rng, id, idPrefix, theme, tier)`** - flavor-only, no archetype row at all (not in `monsters-manifest.json`). Race/class picked freely; biography comes from a `{threat}`-templated hook (`friendlyNpcHooks`) so it still reads as tied to the floor's own threat. Placed by `MonsterGenerator.mjs`'s `placeFriendlyNpcs()` in an `entrance`/`rest-area` room (the two roles mob placement always leaves empty) with `disposition: 1` (friendly), one per floor. No quest/reward/dialogue mechanics - the name/bio is the whole feature.

**Level formula** (the one difficulty knob, same spirit as Monsters' `cr`): `clamp(round(tier * multiplier), 1, 20)` where boss=4, elite=2, friendly NPC=1 (untiered - they're not built to fight).

**Build pipeline (same for all three), reusing already-generated content, never hand-derived:**
1. Race picked by matching `primaryAbility` against each race's largest `abilityBonuses` entry (falls back to any race).
2. Class picked by matching `primaryAbility`, biased toward caster classes (`manaPerLevel >= staminaPerLevel`) when the archetype's `combatSkill` is Cast/Channel (falls back to any class matching `primaryAbility`, then any class). Friendly NPCs pick without a caster bias.
3. Race/class **items are cloned directly from the already-generated `src/packs/races/`/`src/packs/classes/` (not re-derived from the manifest)** - guarantees they can never drift from what the shared compendiums actually contain. **Their `grantedFeatures` are also resolved and cloned as real embedded Feature items** (`resolveGrantedFeatures()`), mirroring what `actor-sheet.mjs`'s `_grantFeaturesFromItem` does for a live player dragging a class onto their sheet - a class's `grantedFeatures` list has no mechanical effect on its own (only items actually embedded on the actor get aggregated - see `base-actor.mjs`'s `_aggregateSkills`/`_aggregateLuck`), so skipping this step would silently under-deliver a class's real features.
4. One weapon + one armor cloned from `src/packs/weapons`/`armor`, `equipped: true`, biased toward higher `rarity` at higher `level` (falls back down the rarity ladder - the non-`common` pool is thin, ~4 weapons/3 armor pieces above common today, so most builds land on common gear; that's fine, variety mostly comes from the race×class×spell×feat combination, not gear rarity).
5. If the picked class is a caster, 1-4 spells cloned from `src/packs/spells` filtered by `castStat` matching the class's `primaryAbility`/`secondaryAbility` (spells' `castStat` is only ever `int`/`wis` - a cha-primary caster class like Bard/Sorcerer with no int/wis secondary correctly gets none; this is an existing content gap, not something this generator can fix).
6. `details.subclass` gets a flavor-only epithet (`the {adjective}`, from the floor's own theme category) - **not a mechanical subclass system**, since none exists yet (`item-class.mjs`'s `subclasses` field and `details.subclass` are both unused free text). Isolated in its own `pickSubclassFlavor()` so it's a one-function swap when real subclass mechanics land.

**HP/Stamina/Mana are never computed here** (unlike Monsters' CR formula) - `hp`/`stamina`/`mana` `.value`/`.max` are written as a large placeholder (999), and `dccworldCharacter#prepareDerivedData()`'s existing self-clamp (`if (this.hp.value > this.hp.max) this.hp.value = this.hp.max`, same for stamina/mana) corrects it down to the real class+level-computed max the moment the actor first loads in Foundry - exactly like a real PC, and immune to the formula ever drifting out of sync between this repo and `Dungeon-Crawler-World`.

**Token bar attributes matter here:** `dccworldCharacter` uses `hp`/`stamina` (not `health`/`power`, which `dccworldNPC` uses) - `wrapActor()`/`buildTokenEnvelope()` both take a `bar1Attribute`/`bar2Attribute` override for this (defaulting to `health`/`power` so Monster mobs are unaffected); every Character actor/token passes `hp`/`stamina` explicitly. Getting this wrong renders a blank health bar.

**Same compendium-import constraint as Monsters, now for two packs:** a GM must import **both** Monsters and Characters into their world (compendium sidebar → right-click → **Import All Content** → **Keep Document IDs**) before opening a generated floor scene, and in that order if possible - see the Monsters section above for why order matters (a scene's tokens resolve their actor at load time, not lazily).

**Adding/changing an elite or boss archetype:** same as **Adding a monster** above (edit `data/monsters-manifest.json`'s `elite`/`boss` rows) - the art/theme/primaryAbility/combatSkill/biography fields work identically, they just no longer produce their own standalone Monster actor. Regenerate with `npm run generate:floors` (there's no separate `generate:characters` script - Characters are built as part of floor generation, see `generate-floors.mjs`).
