...
- New `rules` compendium: a single "Rules Reference" JournalEntry with 41 cross-linked pages
  covering the full player-facing ruleset (core loop, character creation/leveling, all seven
  ability scores, skills, difficulty tiers, HP/Mana/Stamina, combat, status effects, magic, loot,
  achievements, worship, and sample feats). Player-readable by default (`OBSERVER`, unlike the
  `LIMITED` floor journals) since it's meant to be opened at the table for rules lookups, not
  GM-gated content. Generated by `scripts/generate-rules-journal.mjs` from source text adapted
  from `Dungeon-Crawler-World/Rules/**/*.md`, packed by `scripts/pack-rules.mjs`.
